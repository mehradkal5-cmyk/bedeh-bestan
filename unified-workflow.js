...
(function () {
  ...
  const accountName = () => user?.user_metadata?.display_name || user?.email?.split('@')[0] || 'رفیق';
  const typeIcon = (type) => ({ item:'package', money:'coins', expense:'receipt' })[type] || 'notebook';
  ...
  function recordCards(records) {
    if (!records.length) return `<div class="empty"><span class="empty-icon">${icon('notebook')}</span><strong>هنوز چیزی ثبت نشده</strong><small>اولین بده‌بستان این بخش که ثبت شود، همین‌جا می‌بینیش.</small></div>`;
    return `<div class="record-list">${records.map((r) => `<article class="record-card" data-record-type="${r.type}"><div class="record-card-top"><span class="type-label">${icon(typeIcon(r.type))} ${typeName(r.type)} · ${role(r)}</span><span class="chip ${closed(r) ? 'done' : 'open'}">${closed(r) ? 'بسته‌شده' : 'فعال'}</span></div><div class="record-card-body"><h3>${escape(r.title)}</h3><p class="record-person">با ${escape(r.person)}</p>${r.type === 'item' ? '<strong class="record-amount">یک امانت</strong>' : `<strong class="record-amount">${money(balance(r), r)}</strong><small>مانده${r.role === 'contributor' ? 'ٔ سهم تو' : ''}</small>`}</div><div class="record-card-foot"><span>${icon('calendar-blank')} ${date(r.due)}</span><button class="text-btn" data-record="${r.id}">باز کردن ${icon('arrow-left')}</button></div></article>`).join('')}</div>`;
  }
  ...
  function home() {
    ...
    return `<div class="page-head dashboard-head"><div><span class="eyebrow">${active === 'records' ? 'دفتر کامل' : 'نمای امروز'}</span><h1>${active === 'records' ? 'بده‌بستان‌ها' : `سلام ${escape(accountName())}`}</h1><p>${active === 'records' ? 'همهٔ قرض‌ها، دنگ‌ها و امانت‌ها یک‌جا.' : 'حساب رفاقت‌ها مرتب است؛ ببین امروز چه چیزی نیاز به توجه دارد.'}</p></div><button class="primary-btn" data-action="new-record">${icon('plus')} ثبت بده‌بستان</button></div>
      ${active === 'home' ? `<section class="stats" aria-label="خلاصه حساب"><div class="stat stat-credit"><span class="stat-icon">${icon('trend-up')}</span><small>به من بدهکارند</small><strong>${fa(sum(true))}</strong><small>تومان</small></div><div class="stat"><span class="stat-icon">${icon('trend-down')}</span><small>من بدهکارم</small><strong>${fa(sum(false))}</strong><small>تومان</small></div><div class="stat"><span class="stat-icon">${icon('package')}</span><small>امانت‌های باز</small><strong>${fa(open.filter((r) => r.type === 'item').length)}</strong><small>مورد</small></div></section>` : `<div class="records-toolbar"><span>${fa(list.length)} مورد</span><div class="tabs">${[['','همه'],['item','امانت'],['money','قرض'],['expense','دنگ']].map(([key,text]) => `<button class="tab ${window.filter === key ? 'active' : ''}" data-workflow-filter="${key}">${text}</button>`).join('')}</div></div>`}
      ...
      <div class="dashboard-groups"><section class="section ledger-section"><div class="section-heading"><div><span class="section-kicker">من ثبت کردم</span><h2>طلب‌ها و امانت‌های من</h2></div><span class="section-count">${fa(list.filter((r) => r.role === 'creator').length)}</span></div>${recordCards(list.filter((r) => r.role === 'creator'))}</section><section class="section ledger-section"><div class="section-heading"><div><span class="section-kicker">برای من فرستاده شده</span><h2>تعهدهای من</h2></div><span class="section-count">${fa(list.filter((r) => r.role !== 'creator').length)}</span></div>${recordCards(list.filter((r) => r.role !== 'creator'))}</section></div>`;
  }

  function detailPage(r) {
    const manage = r.permissions?.manage;
    return `<button class="back" data-go="home">${icon('arrow-right')} بازگشت</button><div class="detail-head detail-hero"><div><span class="eyebrow">${typeName(r.type)} · ${role(r)}</span><h1 class="detail-title">${escape(r.title)}</h1><p>${escape(r.person)} · سررسید ${date(r.due)}</p></div><div class="detail-summary"><span class="chip ${closed(r) ? 'done' : 'open'}">${closed(r) ? 'بسته‌شده' : 'فعال'}</span>${r.type !== 'item' ? `<strong>${money(balance(r),r)}</strong><small>مانده</small>` : ''}</div></div>
  ...
  function notificationsPage() {
    return `<div class="page-head"><div><span class="eyebrow">صندوق خبرها</span><h1>اعلان‌ها</h1><p>درخواست‌ها و تغییرات تازهٔ حساب.</p></div><button class="text-btn" data-read-all>خواندن همه</button></div>${notificationsError ? `<p class="field-error" role="alert">${escape(notificationsError)} <button class="text-btn" data-sync>تلاش دوباره</button></p>` : ''}<div class="notification-feed">${state.notifications.map((n) => {
    ...
    }).join('') || (!notificationsError ? `<div class="empty"><span class="empty-icon">${icon('bell-slash')}</span><strong>همه‌چیز آرومه</strong><small>اعلان تازه‌ای نداری.</small></div>` : '')}</div>`;
  }

  function settings() {
    return `<div class="page-head"><div><span class="eyebrow">فضای شخصی</span><h1>تنظیمات</h1><p>حساب، ظاهر و یادآوری‌ها را اینجا تنظیم کن.</p></div></div><div class="settings-grid"><section class="info-block settings-profile"><span class="profile-avatar">${escape(accountName().slice(0,1))}</span><div><small>حساب فعال</small><strong>${escape(accountName())}</strong><p dir="ltr">${escape(user?.email || '')}</p></div><button class="secondary-btn" data-account-action>مدیریت حساب</button></section><section class="info-block"><h2>ظاهر و بازخورد</h2><div class="setting-row"><div><strong>حال‌وهوای برنامه</strong><p>بین دفتر روشن و شبانه جابه‌جا شو.</p></div><button class="secondary-btn" data-theme-setting aria-pressed="${document.documentElement.dataset.theme === 'dark'}">${document.documentElement.dataset.theme === 'dark' ? 'روشنش کن' : 'تاریکش کن'}</button></div><div class="setting-row"><div><label for="wf-sound"><strong>صدای اعلان</strong></label><p>برای خبرهای تازه یک صدای کوتاه پخش شود.</p></div><input id="wf-sound" class="toggle" type="checkbox" data-sound-setting ${window.BedehFriendly.soundEnabled() ? 'checked' : ''}></div></section><section class="info-block"><h2>یادآوری‌ها</h2><div class="setting-row"><div><label for="wf-reminders"><strong>یادآوری موعدها</strong></label><p>بر اساس ساعت تهران پیگیری شود.</p></div><input id="wf-reminders" class="toggle" type="checkbox" data-reminder-pref ${preferences.reminders_enabled ? 'checked' : ''}></div><div class="setting-row setting-stack"><div><strong>اعلان این دستگاه</strong><p>خبرها روی همین دستگاه هم نمایش داده شوند.</p></div><div class="actions"><button class="primary-btn" data-push-enable>فعال‌کردن</button><button class="secondary-btn" data-push-disable>غیرفعال‌کردن</button></div></div><p class="field-error" id="push-status" role="status"></p></section></div>`;
  }

  function content() {
    if (!authReady) return `<section class="auth-landing workflow-auth-loading" aria-busy="true"><div class="auth-copy"><span class="eyebrow">دفتر رفاقت‌ها</span><h1>حسابت سر جاشه</h1><p>${syncError ? escape(syncError) : 'در حال بازیابی حساب و باز کردن دفترت…'}</p>${syncError ? '<button class="secondary-btn" data-sync>تلاش دوباره</button>' : '<span class="auth-loader"></span>'}</div></section>`;
    if (!user) return `<section class="auth-landing"><div class="auth-copy"><span class="eyebrow">حساب‌وکتاب بدون رودربایستی</span><h1>رفاقت سر جاش، حساب هم روشن.</h1><p>قرض، دنگ و امانت را ثبت کن؛ هرکس فقط سهم خودش را می‌بیند و پیگیری می‌کند.</p><button class="primary-btn auth-cta" data-login>${icon('sign-in')} ورود یا ساخت حساب</button><div class="auth-points"><span>${icon('check-circle')} سهم دقیق هر نفر</span><span>${icon('shield-check')} دعوت امن و یک‌بارمصرف</span><span>${icon('bell')} یادآوری بی‌دردسر</span></div></div><div class="auth-ledger" aria-hidden="true"><div class="ledger-sheet"><span class="ledger-pin"></span><small>دفتر این هفته</small><strong>جوجه با بچه‌ها</strong><div><span>رضا</span><b>۲۴۰٬۰۰۰</b></div><div><span>محسن</span><b>۲۴۰٬۰۰۰</b></div><div><span>سارا</span><b>۲۴۰٬۰۰۰</b></div><footer><span>جمع دنگ</span><b>۷۲۰٬۰۰۰ تومان</b></footer></div></div></section>`;
  ...
  function renderSurface(refresh = false) {
    ...
    if (document.querySelector('#main')?.closest('[data-unified]')) {
      ...
      app.innerHTML = `<div class="app-shell" data-unified><header class="topbar"><div class="topbar-inner"><div class="brand-row"><div class="brand-lockup"><div class="brand">بده‌بستان<span></span></div><small>دفتر رفاقت‌ها</small></div><div class="top-actions"><button class="icon-btn" data-action="toggle-dark" aria-label="تغییر تم">${icon('sun')}</button><button class="icon-btn" data-go="notifications" aria-label="اعلان‌ها">${icon('bell')}<span data-unread></span></button></div></div>${user ? `<button class="account-mini" data-account-action><span>${escape(accountName().slice(0,1))}</span><div><strong>${escape(accountName())}</strong><small>حساب من</small></div>${icon('caret-left')}</button>` : ''}<p class="workflow-connection" role="status" data-connection></p></div></header><nav class="nav" aria-label="ناوبری اصلی"><div class="nav-inner">${[['home','خانه','house'],['records','بده‌بستان‌ها','handshake'],['notifications','اعلان‌ها','bell'],['settings','تنظیمات','gear-six']].map(([key,title,i]) => `<button data-go="${key}" class="${active === key ? 'active' : ''}" ${active === key ? 'aria-current="page"' : ''}>${icon(i)} <span>${title}</span></button>`).join('')}</div></nav><div class="workflow-stage"><div data-tip-slot></div><main class="main" id="main">${html}</main></div></div>`;
    }
